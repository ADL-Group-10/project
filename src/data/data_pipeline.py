...
class DataPipeline:
    ...
    def summary(self) -> dict:
        ...
        for split in ("train", "val", "test"):
            ...
            self.logger.info(
                f"{split:>5s}: {num_images:>6d} images | "
                f"{total_bboxes:>6d} bboxes | {avg_bboxes:.2f} avg/img"
            )
    ...
    def _build_pipeline(self, augment: str, snow_config=None) -> A.Compose:
        """
        Build an Albumentations Compose for the given augment mode.

        Note: Resize and Normalize are intentionally excluded.
        Ultralytics handles both internally. Including them here would
        cause double-normalization and is stripped by trainer._attach_albumentations()
        anyway — keeping them out makes the pipeline correct everywhere it's used.
        """
        transforms = []
    ...
    def _setup(self) -> None:
        ...
        # write manifest so _exists() can detect config changes on next run
        manifest_hash = self._compute_manifest_hash()
        (self.output_dir / ".manifest").write_text(manifest_hash)
        self.logger.info(f"Manifest written: {manifest_hash}")
    ...
    def _process_sequence(self, seq_name: str, split: str) -> None:
        ...
        if not mp4_files and not png_files:
            for child in seq_dir.iterdir():
                if child.is_dir():
                    png_files = sorted(
                        f for f in child.iterdir() if f.suffix.lower() == ".png"
                    )
                    if png_files:
                        break
    ...
    def _extract_frames(
        self, mp4_path, frame_annotations, img_dir, lbl_dir, seq_name
    ) -> None:
        import decord
        ...
        self.logger.info(
            f"decord: extracting {len(annotated_indices)} frames from {mp4_path.name}"
        )

        for frame_idx in annotated_indices:
            if frame_idx >= len(vr):
                self.logger.warning(
                    f"Frame {frame_idx} out of range ({len(vr)} total), skipping"
                )
                continue
        ...
        self.logger.info(
            f"decord: done -- {len(annotated_indices)} frames from {mp4_path.name}"
        )

    def _copy_png_frames(
        self, png_files, frame_annotations, img_dir, lbl_dir, seq_name
    ) -> None:
        import shutil
        saved = 0
        for frame_idx, png_path in enumerate(png_files):
            if frame_idx in frame_annotations:
                ...
                self._write_yolo_label(
                    lbl_dir / f"{name}.txt", frame_annotations[frame_idx]
                )
                saved += 1
    ...
    def _find_dir(self, seq_name: str) -> Path:
        ...
        for item in self.raw_dir.iterdir():
            if item.is_dir() and item.stem.lower().replace(" ", "_") == norm:
                return item
        # log available dirs to help diagnose name mismatches
        available = [d.name for d in self.raw_dir.iterdir() if d.is_dir()]
        self.logger.error(f"Available dirs in {self.raw_dir}: {available}")
    ...
    def _validate(self) -> None:
        for split in ("train", "val", "test"):
            ...
            if images - labels:
                raise ValueError(
                    f"{split}: {len(images - labels)} images have no label"
                )
            if labels - images:
                raise ValueError(
                    f"{split}: {len(labels - images)} labels have no image"
                )
            if not images:
            ...
            for lbl_file in lbl_dir.glob("*.txt"):
                with open(lbl_file) as f:
                    for ln, line in enumerate(f, 1):
                        ...
                        if len(parts) != 5:
                            raise ValueError(
                                f"{lbl_file.name}:{ln}: expected 5 values, got {len(parts)}"
                            )
                        if int(parts[0]) >= self.num_classes:
                            raise ValueError(
                                f"{lbl_file.name}:{ln}: invalid class_id {parts[0]}"
                            )
                        if not all(0.0 <= float(v) <= 1.0 for v in parts[1:]):
                            raise ValueError(
                                f"{lbl_file.name}:{ln}: values out of [0,1]"
                            )
        self.logger.info("Validation passed.")

    def _exists(self) -> bool:
        # check all directories exist and are non-empty
        ...
        # check config hasn't changed since last build
        manifest = self.output_dir / ".manifest"
        if not manifest.exists():
            return False
        current = self._compute_manifest_hash()
        return manifest.read_text().strip() == current

    def _compute_manifest_hash(self) -> str:
        """MD5 of the config fields that affect dataset content."""
        return hashlib.md5(json.dumps({
            "splits":      OmegaConf.to_container(self.splits),
...
if __name__ == "__main__":
    ...
    def apply_and_visualize(image, bboxes, class_labels, aug):
        """Apply augmentation and visualize. No normalize/denormalize needed."""
        result = aug(image=image, bboxes=bboxes, class_labels=class_labels)
        return AV.visualize(result["image"], result["bboxes"])

    def wrap_single(transform):
        """Wrap a single transform in a Compose for visualisation."""
        return A.Compose(
            [transform],
            bbox_params=A.BboxParams(
                format="yolo",
    ...
    for col, (label, key) in enumerate([
        ("V1 base", "V1 base"), ("V2 full snow", "V2 full snow")
    ]):
    ...
    for col, (label, key) in enumerate([
        ("V3 train (light)", "V3 train (light)"),
        ("V3 test (heavy)",  "V3 test (heavy)"),
    ]):
